...
Для предотвращения случайного попадания в GitHub ключей и других конфиденциальных данных настроены pre-commit хуки с `GetLeaks`.
...
- **TLS:** Настроен защищенный протокол передачи данных с актуальными наборами шифров (cipher suites).
  - Обеспечивает целостность и конфиденциальность трафика между клиентом и сервером.
